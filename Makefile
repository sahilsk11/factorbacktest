...
deploy-lambda:
	...
	aws lambda update-function-configuration --region us-east-1 --function-name fbTestArm --environment "Variables={commit_hash=$(git rev-parse --short HEAD),GIN_MODE=release}"
